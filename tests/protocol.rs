...
#[test]
fn explicit_v6_wire_format_and_initial_dispatch_are_versioned() -> Result<()> {
    ...
    assert_eq!(
        &wire[4..],
        br#"{"type":"sync","protocol":6,"share":"share","peer":"peer","relationship":"relationship","dry_run":false}"#
    );
    assert!(matches!(
        sync::read_message(&mut wire.as_slice())?,
        Message::Sync { protocol: 6, .. }
    ));
